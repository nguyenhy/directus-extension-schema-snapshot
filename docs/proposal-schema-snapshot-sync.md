- [Feature Proposal: Host-Repo Schema Snapshot Sync](#feature-proposal-host-repo-schema-snapshot-sync)
  ...
  - [5. Implementation Reality Check (post-hoc, written against the actual code on `fix/snapshot-sync`)](#5-implementation-reality-check-post-hoc-written-against-the-actual-code-on-fixsnapshot-sync)
    - [5.1. What the plan got wrong](#51-what-the-plan-got-wrong)
    - [5.2. Implementation notes and trade-offs not visible in code comments](#52-implementation-notes-and-trade-offs-not-visible-in-code-comments)
...
- **§2/§3.2 "`sync`: replay meta.json into local GitStore" (incremental) is not what's implemented.** First implementation _was_ incremental — track synced event ids in `sync-state.json`, only commit new ones. It reproduced exactly the bug §1 exists to fix: `add` already commits directly into GitStore _and_ writes the event; `sync` then saw that event as "unsynced" (nothing recorded it) and re-committed it, doubling every version in `list`. Real fix (`core/operations/sync.js`'s `syncSnapshots`): **`sync` wipes `storeDir` entirely (`wipeStore()` — `rm -rf` + recreate + `git init`) and replays every active event from scratch, every time.** Not incremental, not additive. `sync-state.json` now stores only `{ syncedHash }` (the whole log's content hash, for `status` to compare against) — the `syncedEventIds` array from the first attempt is gone.
- **§3.7 "cache silently becomes stale" fix undersells what "sync" now means.** The proposal frames staleness as a read-time problem solved by a hash comparison gate. In the shipped code, `sync` _is_ the rebuild — there's no partial catch-up path. `status` (§3.8) is correctly read-only, but there is no "only replay what changed" — every `sync` call is a full teardown/rebuild of `.snapshot/repo`.
...
**Update (later in `fix/snapshot-sync`):** the "user must run `sync` manually before a ref resolves" bullet above is now only true for versions committed _without_ `--snapshots-dir`. `add` and `remove --latest` were changed to mint their `meta.json` event id and stamp the GitStore commit message with it _before_ committing (not after, and not only during `sync`'s replay) — so `list`/`resolveRef` see the durable identity the instant either command returns. This closed a real bug: `list` showed `event`/`hash` as `"-"` for any version between the moment it was `add`ed and the next `sync`, because the commit `add` wrote had a plain message, not the `sync: eN (hash)` form only `sync`'s replay used to produce. `sync` itself also changed at the same time: it now replays `meta.json`'s **full event log** (add and remove events, not just the currently-active set), so `list`'s output before and after a `sync` matches exactly, including the removal/undo history — previously `sync` only replayed active `add` events, silently dropping every `remove` from GitStore's visible history. See docs/architecture.md's "Schema-snapshots sync layer" for the current model.
...
**Update (later in `fix/snapshot-sync`): this bullet is fixed for `--latest`, not for `--hash`/`--id`.** `removeLatestVersion` now also appends a `remove` event to `meta.json` — targeting whatever event id is currently at GitStore's HEAD, read off HEAD's own stamped commit message — so `sync` can no longer resurrect what `--latest` just reverted. This also made `--latest` **safely repeatable**: since `remove` events can target _either_ an `add` event or another `remove` event, running `--latest` again on top of a remove commit undoes that specific removal (restoring the content), running it a third time re-removes it, and so on indefinitely — a toggle chain, not a one-shot revert. See docs/architecture.md's event-log model description for the chain/parity mechanics. `--hash`/`--id` is unchanged and still independent of `--latest`'s bookkeeping — it remains capable of tombstoning a non-newest active event, which `sync`'s replay cannot correctly reconstruct (it can only revert current HEAD, never an arbitrary historical commit); `sync` throws in that case rather than silently rebuilding wrong history, but the two mechanisms are still not unified into one model.
